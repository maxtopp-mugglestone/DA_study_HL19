# local copy of lumi.py, sourced from https://github.com/andreafornara/xtrack/
# includes lumi levelling from xlhc to replace 6d twiss with 4d twiss
# copy of functions from misc.py, needed until xt.lumi supports crab cavity
...
def luminosity_leveling_ip1_5(
    ...
    # Get Twiss
    try: 
        twiss_b1 = collider["b1"].twiss()
        twiss_b2 = collider["b2"].twiss()
    except: 
        twiss_b1 = collider["b1"].twiss4d()
        twiss_b2 = collider["b2"].twiss4d()

    # Get the number of colliding bunches in IP1/5
    n_colliding_IP1_5 = config_collider["lumi_leveling"]["ip1_5"]["num_colliding_bunches"]

    # Get max intensity in IP1/5
    max_intensity_IP1_5 = float(
        config_collider["lumi_leveling"]["ip1_5"]["constraints"]["max_intensity"]
    )

    def compute_lumi(bunch_intensity):
        luminosity = luminosity_from_twiss(  # type: ignore
    ...
    def f(bunch_intensity):
        ...
        max_PU_IP_1_5 = config_collider["lumi_leveling"]["ip1_5"]["constraints"]["max_PU"]
        target_luminosity_IP_1_5 = config_collider["lumi_leveling"]["ip1_5"]["luminosity"]
        PU = compute_PU(
            luminosity,
            n_colliding_IP1_5,
            twiss_b1["t_rev0"],
